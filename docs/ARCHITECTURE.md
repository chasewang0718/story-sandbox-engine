...
为支持长篇叙事中的人格一致性与人物转变，角色状态需显式包含下列可计算参数：

- **人物性格（Traits）**：如谨慎、冲动、利他等连续值或离散标签。
- **人物目标（Goals）**：短/中/长期目标及优先级。
- **人物动机（Motives）**：求生、复仇、守护、权力等权重向量。
- **人物弧光（Arc）**：阶段、触发器、阈值与可逆/不可逆转变规则。

该层的使用原则：

- 参数进入世界真源并随事件更新，不只存在于文案。
- LLM 在意图生成时读取这些参数，但输出仍须结构化并受 schema 校验。
- 裁判层执行「角色内一致性」检查，避免无触发的突兀黑化/洗白。
- 弧光推进由触发规则驱动（阈值达成再转段），并写入事件日志以便回放与审计。
